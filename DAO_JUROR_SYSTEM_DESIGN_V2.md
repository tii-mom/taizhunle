...
| 积分范围 | 称号 | 质押金额 (TAI) | 创建冷却 | 每日陪审上限 |
|---------|------|---------------|-----------|---------------|
| 普通用户 | - | 1,000 | 360 小时 | 不可陪审 |
| 0-99     | 武林新丁 (Lv.1) | 1,000 | 72 小时 | 3 次 |
| 100-399  | 风尘奇侠 (Lv.2) | 5,000 | 48 小时 | 9 次 |
| 400-999  | 地狱判官 (Lv.3) | 10,000 | 24 小时 | 30 次 |
| 1000+    | 天上天下天地无双 (Lv.4) | 20,000 | 6 小时 | 无限制 |

- 质押在预测结算后返还；申诉成立扣 50%，恶意创建扣 100%，超时未结算扣 20%。
- 奖池实时奖励 = 奖池总额 × 1%，按陪审员人数平分。

### 设计理念

1. **防止垃圾预测**：普通用户冷却 360 小时且只能质押 1,000 TAI，有效阻断低质量预测。
2. **鼓励成为陪审员**：等级越高冷却越短，形成“陪审 → 积分 → 更多创建权限”的正循环。
3. **经济安全**：质押作为保证金，发生申诉或恶意行为时用于赔付。
4. **运营友好**：质押与冷却明晰，易于在前端和文档中展示。
